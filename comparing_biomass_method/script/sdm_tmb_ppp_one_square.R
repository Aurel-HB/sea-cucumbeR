...
data_used <- data_position[data_position$station == 96,]
...
stat_square <- shp_grid[shp_grid$Id == 96,]
stat_square <- stat_square[!is.na(stat_square$Id),]
stat_square <- as.data.frame((stat_square["96", 6])[[1]][[1]][[1]])
names(stat_square) <- c("long","lat")

# zeros is generated on a grid for this example, but other strategies could be used
zeros <- expand.grid(
  x = seq(min(stat_square$long), max(stat_square$long), by = res),
  y = seq(min(stat_square$lat), max(stat_square$lat), by = res)
)
