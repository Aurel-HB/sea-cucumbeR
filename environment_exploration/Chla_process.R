#This code extract the Mass concentration of chlorophyll-a in sea water from 
# a NetCDF file and process the data to keep the period between 2021 and 2025 
# in the study area : (45.849N, 45.3656S, -56.1402E, -56.26W)
...
ncrast <- raster::stack(paste(here(),
                              ...
                              "cmems_mod_glo_bgc_my_0.25deg_P1M-m_1767881042352.nc",
                              sep=""), varname = "chl")

print(ncrast)

# Get a data.frame with raster cell values ####
chl_df <- as.data.frame(ncrast, xy = TRUE) 
...
names(chl_df) <- c("lon","lat",name)


# transform dataframe in sf
mass_chl <- st_as_sf(chl_df, crs = "EPSG:4326", coords = c("lon","lat"))


#map the chl ####
ggplot(mass_chl['May_2025'])+
...
chl_df_SPM <- as.data.frame(ncrast_SPM, xy = TRUE) 
names(chl_df_SPM) <- c("lon","lat",name)

plot(ncrast_SPM)

# transform dataframe in sf
mass_chl_SPM <- st_as_sf(chl_df_SPM, crs = "EPSG:4467",
                              coords = c("lon","lat"))

#map the chl #
ggplot(mass_chl_SPM['May_2025'])+
  geom_sf(aes(color=May_2025))+
  scale_color_viridis()+
...
mass_chl_SPM <- st_as_sf(ncstars, crs = "EPSG:4467",
                              coords = c("lon","lat"))
names(mass_chl_SPM) <- c(name,"geometry")
saveRDS(mass_chl_SPM,
        paste(here(),"/environment_exploration/Environment_Data/processed/",
              "Chla_500.rds",sep = ""))

# finally join the prediction grid with the mass_chl_SPM sf
grid_chl <- 
  st_join(grid, mass_chl_SPM) # to get intersection of points and poly
