...
$$\hat{z}\text{ is the simulated variable and } z \text{ is the true observed variable}$$
$$\text{RMSD} = \sqrt{\frac{1}{n}\sum_{i=1}^{n}(\hat{z}(s_i)-z(s_i))^2}$$
$$\text{MAE} = \frac{1}{n}\sum_{i=1}^{n}|\hat{z}(s_i)-z(s_i)|$$
...
ggplot(data_RMSD_MAE)+
  ...
  geom_hline(yintercept = mean(data_RMSD_MAE$RMSD), color = "green")

ggplot(data_RMSD_MAE)+
  geom_boxplot(aes(x = model, y = MAE), 
               outlier.colour="black", outlier.shape=16,
             outlier.size=2, notch=FALSE)+
  geom_hline(yintercept = mean(data_RMSD_MAE$MAE), color = "green")
...
data.frame(
  "AIC"=c("Lognormal", AIC(fit_tot_logN)),
  "AIC"=c("Lognormal_chla", AIC(fit_tot_logN_chla)),
  "AIC"=c("Lognormal_bat_temp_chla", AIC(fit_tot_logN_cov))
)
```
