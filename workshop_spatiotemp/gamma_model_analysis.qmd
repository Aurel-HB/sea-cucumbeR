...
required_packages <- c("leaflet","dplyr","sp",
                       "ggplot2","sf", "sdmTMB", "here",
                       "INLA","viridis","DHARMa","future","gstat","ggpubr")
...
# Model Gamma to estimate abundance
```{r,include=F}
...
for(col in 3:1002){
  ...
  data_RMSD_MAE <- rbind(data_RMSD_MAE,
                         data.frame(Gamma_RMSD,Gamma_MAE))
}

data_RMSD_MAE <- data.frame(
  RMSD = c(data_RMSD_MAE$Gamma_RMSD),
  MAE = c(data_RMSD_MAE$Gamma_MAE),
  model = c(rep("Gamma",1000))
)
...
```{r,echo=F}
simulated <- rowMeans(gamma_residual[,c(3,1002)])
observed <- gamma_residual$observedResponse
...
R_square <- c()
for(col in 3:1002){
  R_square <- c(R_square,cor(gamma_residual$observedResponse,
      gamma_residual[,col],method = "pearson"))
}
